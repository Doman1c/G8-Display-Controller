...
__attribute__((always_inline)) static inline void copy_pixel_16bpp(uint8_t *to, const uint8_t *from) { *(uint16_t *)to++ = *(const uint16_t *)from++; }
__attribute__((always_inline)) static inline void copy_pixel_24bpp(uint8_t *to, const uint8_t *from) { *to++ = *from++; *to++ = *from++; *to++ = *from++; }
...
static lv_port_flush_probe_t flush_copy_probe(lv_disp_drv_t *drv)
{
    ...
    if (prev_status == FLUSH_STATUS_FULL) {
        probe_result = (cur_status == FLUSH_STATUS_PART) ? FLUSH_PROBE_FULL_COPY : FLUSH_PROBE_SKIP_COPY;
    } else {
...
void flush_callback(lv_disp_drv_t *drv, const lv_area_t *area, lv_color_t *color_map)
{
    ...
    if (lcd->getBus()->getBasicAttributes().type == ESP_PANEL_BUS_TYPE_RGB) lv_disp_flush_ready(drv);
}
...
static void update_callback(lv_disp_drv_t *drv)
{
    ...
    switch (drv->rotated) {
    case LV_DISP_ROT_NONE: lcd->swapXY(disp_init_swap_xy); lcd->mirrorX(disp_init_mirror_x); lcd->mirrorY(disp_init_mirror_y); break;
    case LV_DISP_ROT_90: lcd->swapXY(!disp_init_swap_xy); lcd->mirrorX(disp_init_mirror_x); lcd->mirrorY(!disp_init_mirror_y); break;
    case LV_DISP_ROT_180: lcd->swapXY(disp_init_swap_xy); lcd->mirrorX(!disp_init_mirror_x); lcd->mirrorY(!disp_init_mirror_y); break;
    case LV_DISP_ROT_270: lcd->swapXY(!disp_init_swap_xy); lcd->mirrorX(!disp_init_mirror_x); lcd->mirrorY(disp_init_mirror_y); break;
    }
}
...
void rounder_callback(lv_disp_drv_t *drv, lv_area_t *area)
{
    ...
    if (x_align > 1) { area->x1 &= ~(x_align - 1); area->x2 = (area->x2 & ~(x_align - 1)) + x_align - 1; }
    if (y_align > 1) { area->y1 &= ~(y_align - 1); area->y2 = (area->y2 & ~(y_align - 1)) + y_align - 1; }
}
...
static lv_disp_t *display_init(LCD *lcd)
{
    ...
    for (int i = 0; (i < LVGL_PORT_DISP_BUFFER_NUM) && (i < LVGL_PORT_BUFFER_NUM_MAX); i++) lvgl_buf[i] = lcd->getFrameBufferByIndex(i);
#endif
#endif
    ...
    if (lcd->getBasicAttributes().basic_bus_spec.isFunctionValid(LCD::BasicBusSpecification::FUNC_SWAP_XY) &&
        ...
        lcd->getBasicAttributes().basic_bus_spec.isFunctionValid(LCD::BasicBusSpecification::FUNC_MIRROR_Y)) disp_drv.drv_update_cb = update_callback;
    else disp_drv.sw_rotate = 1;
#endif
    disp_drv.draw_buf = &disp_buf;
    disp_drv.user_data = (void *)lcd;
    if ((lcd->getBasicAttributes().basic_bus_spec.x_coord_align > 1) ||
        (lcd->getBasicAttributes().basic_bus_spec.y_coord_align > 1)) disp_drv.rounder_cb = rounder_callback;
...
static void touchpad_read(lv_indev_drv_t *indev_drv, lv_indev_data_t *data)
{
    ...
    if (read_touch_result > 0) { data->point.x = point.x; data->point.y = point.y; data->state = LV_INDEV_STATE_PRESSED; }
...
static bool tick_init(void)
{
    ...
    ESP_UTILS_CHECK_ERROR_RETURN(esp_timer_create(&lvgl_tick_timer_args, &lvgl_tick_timer), false, "Create LVGL tick timer failed");
...
static void lvgl_port_task(void *arg)
{
    ...
    while (1) {
        if (lvgl_port_lock(-1)) { task_delay_ms = lv_timer_handler(); lvgl_port_unlock(); }
...
bool lvgl_port_init(LCD *lcd, Touch *tp)
{
...
#if LVGL_PORT_AVOID_TEAR
    ESP_UTILS_CHECK_FALSE_RETURN((bus_type == ESP_PANEL_BUS_TYPE_RGB) || (bus_type == ESP_PANEL_BUS_TYPE_MIPI_DSI), false,
                                 "Avoid tearing function only works with RGB/MIPI-DSI LCD now");
    ESP_UTILS_LOGI("Avoid tearing is enabled, mode: %d, rotation: %d", LVGL_PORT_AVOID_TEARING_MODE, LVGL_PORT_ROTATION_DEGREE);
#endif
    ...
    if (bus_type != ESP_PANEL_BUS_TYPE_RGB) lcd->attachDrawBitmapFinishCallback(onDrawBitmapFinishCallback, (void *)disp->driver);
    if (tp != nullptr) {
        indev = indev_init(tp);
        ...
        tp->swapXY(!transformation.swap_xy); tp->mirrorY(!transformation.mirror_y);
#elif LVGL_PORT_ROTATION_DEGREE == 180
        tp->mirrorX(!transformation.mirror_x); tp->mirrorY(!transformation.mirror_y);
#elif LVGL_PORT_ROTATION_DEGREE == 270
        tp->swapXY(!transformation.swap_xy); tp->mirrorX(!transformation.mirror_x);
#endif
#endif
    }
...
bool lvgl_port_deinit(void)
{
    ...
    if (lvgl_task_handle != nullptr) { vTaskDelete(lvgl_task_handle); lvgl_task_handle = nullptr; }
    ...
    for (int i = 0; i < LVGL_PORT_BUFFER_NUM; i++) {
        if (lvgl_buf[i] != nullptr) { free(lvgl_buf[i]); lvgl_buf[i] = nullptr; }
    }
#endif
    if (lvgl_mux != nullptr) { vSemaphoreDelete(lvgl_mux); lvgl_mux = nullptr; }
    return true;
}
